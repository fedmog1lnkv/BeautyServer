...
public class GetUserCouponsByIdQueryHandler(ICouponReadOnlyRepository couponReadOnlyRepository)
    : IQueryHandler<GetUserCouponsByIdQuery,
        Result<PaginatedVm<CouponLookupDto>>>
{
    public async Task<Result<PaginatedVm<CouponLookupDto>>> Handle(
        GetUserCouponsByIdQuery request,
        CancellationToken cancellationToken)
    {
        var coupons = await couponReadOnlyRepository.GetByUserId(request.Id, cancellationToken);
        ...
        var couponDtos = coupons.Select(
            s => new CouponLookupDto
            {
                Id = s.Id,
